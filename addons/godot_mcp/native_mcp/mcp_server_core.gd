...
func _handle_tool_call(message: Dictionary) -> Dictionary:
	...
	var arguments: Dictionary = _coerce_tool_arguments(params.get("arguments", {}))
...
## 部分 MCP 客户端会把空参数发成 [] 而不是 {}，这里统一转成 Dictionary。
func _coerce_tool_arguments(raw: Variant) -> Dictionary:
	...
	if raw is Array:
		if raw.is_empty():
			return {}
		_log_warn("Tool arguments must be a Dictionary, got non-empty Array. Using empty dictionary.")
		return {}
	_log_warn("Tool arguments must be a Dictionary, got: " + type_string(typeof(raw)) + ". Using empty dictionary.")
	return {}
